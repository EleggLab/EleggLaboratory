...
function collectIssues(strict) {
  const issues = [];
  const appName = readEnv('TOSS_APP_NAME', 'bounce-stack');
  const consoleAppName = readEnv('TOSS_CONSOLE_APP_NAME', appName);
  const displayName = readEnv('TOSS_BRAND_DISPLAY_NAME', '바운스 스택');
  const primaryColor = readEnv('TOSS_BRAND_PRIMARY_COLOR', '#1664FF');
  const iconUrl = readEnv('TOSS_BRAND_ICON_URL', 'https://placehold.co/600x600/png?text=Bounce+Stack');
...
if (issues.length === 0) {
  console.log(strict ? 'Bounce Stack release env validation passed.' : 'Bounce Stack env sanity check passed.');
  process.exit(0);
}

console.error(strict ? 'Bounce Stack release env validation failed:' : 'Bounce Stack env sanity check found issues:');
