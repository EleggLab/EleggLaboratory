# Starter Change Report

Generated: 2026-03-14 11:26:05 KST

## Git Status
 m samples
 M starter/starter-report.md
?? research/src/
?? starter/backlog.md
?? starter/checklists/research-ingestion-checklist.md
?? starter/docs/patterns/adoption-policy.md
?? starter/scripts/apply-adoption-batch.py
?? starter/scripts/refresh-research.sh

## Last Commit
- c78568b Add top30 pattern synthesis and starter adoption triage pipeline (OpenClaw Assistant, 2분 전)
## Key Files
starter/.github/workflows/starter-ci.yml
starter/README.md
starter/ROADMAP.md
starter/backlog.md
starter/changelog/CHANGELOG.md
starter/checklists/agent-run-checklist.md
starter/checklists/release-checklist.md
starter/checklists/research-ingestion-checklist.md
starter/checklists/security-checklist.md
starter/common/reference-top1.md
starter/docs/ADR.template.md
starter/docs/PRD.template.md
starter/docs/i18n/glossary.md
starter/docs/i18n/locale-fallback-policy.md
starter/docs/i18n/translation-style-guide.md
starter/docs/patterns/adoption-policy.md
starter/docs/patterns/top30-synthesis.md
starter/docs/postmortem.template.md
starter/examples/demo-bot/README.md
starter/examples/demo-bot/bot.py
starter/examples/demo-cli/README.md
starter/examples/demo-cli/main.sh
starter/examples/demo-cli2/README.md
starter/examples/demo-cli2/main.sh
starter/examples/demo-vibe/.env.example
starter/examples/demo-vibe/.gitignore
starter/examples/demo-vibe/README.md
starter/examples/demo-web/README.md
starter/examples/demo-web/app.js
starter/examples/demo-web/index.html
starter/examples/demo-web/styles.css
starter/examples/request-examples.md
starter/install.sh
starter/locales/README.md
starter/locales/en/README.md
starter/locales/ja/README.md
starter/locales/ko/README.md
starter/locales/zh/README.md
starter/package.json
starter/prompts/reference/context-engineering-cards.md
starter/prompts/reference/foundation-prompts.md
starter/prompts/system.base.md
starter/prompts/task.template.md
starter/prompts/translation.prompt.md
starter/scripts/apply-adoption-batch.py
starter/scripts/bootstrap.sh
starter/scripts/build-locales-index.sh
starter/scripts/init.sh
starter/scripts/new-task.sh
starter/scripts/refresh-research.sh
starter/scripts/release-tag.sh
starter/scripts/report.sh
starter/scripts/research-github-deep.py
starter/scripts/research-github.py
starter/scripts/triage-research-cards.py
starter/scripts/verify.sh
starter/scripts/vibe-starter.js
starter/starter-report-node.md
starter/starter-report.md
starter/templates/bot/README.md
starter/templates/bot/bot.py
starter/templates/cli/README.md
starter/templates/cli/main.sh
starter/templates/webapp/README.md
starter/templates/webapp/app.js
starter/templates/webapp/index.html
starter/templates/webapp/styles.css
