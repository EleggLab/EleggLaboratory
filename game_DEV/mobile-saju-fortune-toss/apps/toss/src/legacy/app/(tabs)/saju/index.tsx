...
import { Button, TextField, Toast } from '@toss/tds-react-native';
        ...
        <SectionCard title="사주 입력">
          ...
          <View style={styles.choiceRow}>
            <View style={styles.choiceCell}>
                <Button
                  ...
                  size="medium"
                  style={calendar === 'solar' ? 'fill' : 'weak'}
                  type={calendar === 'solar' ? 'primary' : 'dark'}
                >
                  양력
                </Button>
            </View>
            <View style={styles.choiceCell}>
                <Button
                  ...
                  size="medium"
                  style={calendar === 'lunar' ? 'fill' : 'weak'}
                  type={calendar === 'lunar' ? 'primary' : 'dark'}
                >
                  음력
                </Button>
            </View>
          ...
          <View style={styles.choiceRow}>
            {([
              ...
              <View key={key} style={styles.choiceCell}>
                <Button
                  display="full"
                  onPress={() => setGender(key)}
                  size="medium"
                  style={gender === key ? 'fill' : 'weak'}
                  type={gender === key ? 'primary' : 'dark'}
                >
                  {label}
                </Button>
              </View>
          ...
          <Button display="full" onPress={() => setAdvancedOpen((v) => !v)} size="medium" style="weak" type="dark">
            {advancedOpen ? '상세 설정 닫기' : '상세 설정 열기'}
          </Button>
          ...
          <Button display="full" onPress={compute} size="medium" style="fill" type="primary">
            사주 풀이 보기
          </Button>
        ...
        <>
          <View style={styles.topRow}>
            <View style={styles.topButton}>
              <Button display="full" onPress={() => setViewMode('report')} size="tiny" style={viewMode === 'report' ? 'fill' : 'weak'} type={viewMode === 'report' ? 'primary' : 'dark'}>
                종합
              </Button>
            </View>
            <View style={styles.topButton}>
              <Button display="full" onPress={() => setViewMode('qna')} size="tiny" style={viewMode === 'qna' ? 'fill' : 'weak'} type={viewMode === 'qna' ? 'primary' : 'dark'}>
                분야별 Q&A
              </Button>
            </View>
            <View style={[styles.topButton, styles.topButtonTrailing]}>
              <Button display="full" onPress={() => setChart(null)} size="tiny" style="weak" type="dark">
                다시 입력
              </Button>
            </View>
          </View>
          ...
          {viewMode === 'report' ? (
                ...
                <View style={styles.inlineInputs}>
                  <View style={styles.inlineField}>
                    <TextField
                      ...
                      label="연도"
                      labelOption="sustain"
                      onChangeText={(text) => setYearText(onlyDigits(text, 4))}
                      placeholder="2026"
                      value={yearText}
                      variant="line"
                    />
                  ...
                  <View style={styles.inlineAction}>
                    <Button display="full" onPress={handlePickYear} size="medium" style="weak" type="dark">
                      해 운세
                    </Button>
                  </View>
                </View>
                <View style={styles.inlineInputs}>
                  <View style={styles.inlineField}>
                    <TextField
                      ...
                      label="월"
                      labelOption="sustain"
                      onChangeText={(text) => setMonthText(onlyDigits(text, 2))}
                      placeholder="2"
                      value={monthText}
                      variant="line"
                    />
                  ...
                  <View style={styles.inlineAction}>
                    <Button display="full" onPress={handlePickMonth} size="medium" style="weak" type="dark">
                      월 운세
                    </Button>
                  </View>
                </View>
            ...
            <SectionCard title="분야별 Q&A">
              ...
              <View style={styles.choiceRow}>
                {([
                  ...
                  <View key={key} style={styles.choiceCell}>
                    <Button
                      display="full"
                      onPress={() => setQnaMode(key)}
                      size="tiny"
                      style={qnaMode === key ? 'fill' : 'weak'}
                      type={qnaMode === key ? 'primary' : 'dark'}
                    >
                      {label}
                    </Button>
                  </View>
              ...
              <View style={styles.domainRow}>
                {DOMAINS.map((item) => (
                  <View key={item.key} style={styles.domainButton}>
                    <Button
                      display="full"
                      onPress={() => setDomain(item.key)}
                      size="tiny"
                      style={domain === item.key ? 'fill' : 'weak'}
                      type={domain === item.key ? 'primary' : 'dark'}
                    >
                      {item.label}
                    </Button>
                  </View>
...
const styles = StyleSheet.create({
  ...
  choiceCell: { flex: 1, minWidth: 92 },
  fieldGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  fieldWide: { flexBasis: '100%', flexGrow: 1 },
  fieldHalf: { flex: 1, minWidth: 120 },
  ...
  topButton: { minWidth: 112 },
  topButtonTrailing: { marginLeft: 'auto' },
  ...
  inlineField: { flex: 1, minWidth: 140 },
  inlineAction: { minWidth: 120 },
  domainRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  domainButton: { minWidth: 96 },
