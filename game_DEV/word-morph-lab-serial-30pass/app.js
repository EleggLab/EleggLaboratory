const STORAGE_KEY = "word-morph-lab-serial-30pass-v1";
...
const CATEGORY_SPEAKERS = {
  celestial: "SUBJECT-01 / LUX",
  elemental: "SUBJECT-01 / FLUX",
  botanical: "SUBJECT-01 / GROWTH",
  arcane: "SUBJECT-01 / RITE",
  creature: "SUBJECT-01 / FERAL",
  gothic: "SUBJECT-01 / VEIL",
  cyber: "SUBJECT-01 / SIGNAL",
  roles: "SUBJECT-01 / MASK",
  moods: "SUBJECT-01 / HEART",
  materials: "SUBJECT-01 / SHELL",
};
...
const CATEGORY_DISPLAY = {
  celestial: "천체 / Celestial",
  elemental: "원소 / Elemental",
  botanical: "식생 / Botanical",
  arcane: "비의 / Arcane",
  creature: "생물 / Creature",
  gothic: "고딕 / Gothic",
  cyber: "사이버 / Cyber",
  roles: "역할 / Roles",
  moods: "감정 / Moods",
  materials: "질감 / Materials",
};
...
const els = {
  ...
  heroMisses: document.getElementById("hero-misses"),
  heroHints: document.getElementById("hero-hints"),
  sessionFilter: document.getElementById("session-filter"),
  sessionHint: document.getElementById("session-hint"),
  sessionBest: document.getElementById("session-best"),
  sessionLastInput: document.getElementById("session-last-input"),
  sessionResonance: document.getElementById("session-resonance"),
  ...
  stageAliases: document.getElementById("stage-aliases"),
  ...
  detailDrift: document.getElementById("detail-drift"),
...
function normalizeSave() {
  ...
  if (!Array.isArray(state.save.eventHistory)) {
    state.save.eventHistory = [];
  } else {
    state.save.eventHistory = state.save.eventHistory.filter((entry) => typeof entry?.type === "string").slice(0, 14);
  }
  ...
  ["missCount", "hintCount", "emptyCount", "archiveJumpCount", "categoryJumpCount"].forEach((key) => {
    if (typeof state.save[key] !== "number" || Number.isNaN(state.save[key])) {
      state.save[key] = 0;
    }
  });
...
function renderHero() {
  ...
  els.heroMisses.textContent = String(state.save.missCount || 0);
  els.heroHints.textContent = String(state.save.hintCount || 0);
}

function renderSessionStrip() {
  const activeFilter = state.save.archiveFilter === "all" ? "전체 상태" : `${getCategoryDisplay(state.save.archiveFilter)} 필터`;
  ...
  els.sessionLastInput.textContent = state.save.lastInput || "없음";
  els.sessionResonance.textContent = `${computeResonanceScore()}%`;
}
...
function renderStage() {
  ...
  if (!word) {
    ...
    els.stageAliases.innerHTML = `<span class="tag-chip">별칭 대기</span>`;
    return;
  }
  ...
  els.stageCategory.textContent = getCategoryDisplay(word.category);
...
function renderDialogue() {
  ...
  els.dialogueEvent.style.setProperty("--event-accent", packet.accent);
}
...
function renderHint() {
  ...
  els.hintPanel.innerHTML = `
    <div class="hint-box" style="--hint-accent: ${escapeAttr(hintWord.accent)};">
      ...
      <strong>${escapeHtml(getCategoryDisplay(hintWord.category))}</strong>
      <ul class="hint-list">
            ...
            .map(
              (word) => `
                <button type="button" class="suggestion-chip" data-suggestion-word-id="${escapeAttr(word.id)}">
                  ...
                  <small>${escapeHtml(getCategoryDisplay(word.category))}</small>
                </button>
              `,
            )
            .join("")}
  ...
  els.categoryChipList.innerHTML = categories
    .map((category) => {
      ...
      return `
        ...
        >
          <span>
            <strong>${escapeHtml(getCategoryDisplay(category.slug))}</strong>
...
function renderDetails() {
  ...
  if (!word) {
    ...
    els.detailAliases.textContent = "-";
    els.detailDrift.textContent = "-";
    els.detailTrigger.textContent = "-";
    els.detailNext.textContent = "-";
    return;
  }

  els.detailWord.textContent = word.label;
  els.detailCategory.textContent = getCategoryDisplay(word.category);
  ...
  els.detailAliases.textContent = formatAliases(word);
  els.detailDrift.textContent = buildDriftAxis(word);
  els.detailTrigger.textContent = buildRecentTrigger(word);
      ...
      return `
        ...
        >
          <span class="recent-item-copy">
            <strong>${escapeHtml(word.label)}</strong>
            <small>${escapeHtml(getCategoryDisplay(word.category))} · ${escapeHtml(word.mood)}</small>
          </span>
...
function renderEventFeed() {
  if (!state.save.eventHistory.length) {
    els.eventList.innerHTML = `
      <div class="recent-empty">
        <strong>이벤트 기록이 비어 있습니다</strong>
        <p>입력, 힌트, 오답, 점프가 발생하면 이곳에 순서대로 쌓입니다.</p>
      </div>
    `;
    return;
  }

  els.eventList.innerHTML = state.save.eventHistory
    .slice(0, 8)
    .map((entry) => {
      const word = findWord(entry.wordId);
      const accent = word?.accent || EVENT_ACCENTS[entry.type] || "#77e1ff";
      return `
        <article class="event-item" style="--event-accent: ${escapeAttr(accent)};">
          <div class="event-item-head">
            <div class="event-item-copy">
              <strong>${escapeHtml(EVENT_LABELS[entry.type] || entry.type)}</strong>
              <small>${escapeHtml(buildEventFeedCopy(entry, word))}</small>
            </div>
            <span class="event-badge">#${escapeHtml(String(entry.seq || 0).padStart(2, "0"))}</span>
          </div>
        </article>
      `;
    })
...
function renderArchive() {
  ...
  els.archiveCaption.textContent =
    state.save.archiveFilter === "all"
      ...
      : `${sortedWords.length}개 표시 · ${getCategoryDisplay(state.save.archiveFilter)} 필터`;
...
function buildDialoguePacket(word, eventType, rawInput) {
  ...
  return {
    speaker: `${CATEGORY_SPEAKERS[word.category] || "SUBJECT-01"} · ${word.label}`,
...
function buildIdleDialoguePacket(eventType, rawInput) {
  ...
  if (eventType === "miss") {
    ...
    const filterNote = state.save.archiveFilter === "all" ? "전체 카탈로그를 다시 넓게 더듬어 보세요." : `${getCategoryDisplay(state.save.archiveFilter)} 필터 안에서 더 가까운 단어를 찾으세요.`;
    return {
      speaker: "LAB CONSOLE",
      event: EVENT_LABELS.miss,
      line: `"${safeInput}"은 표면에서 미끄러졌습니다. 조금 더 가까운 단어가 필요합니다.`,
      directive: "가까운 후보를 누르거나 힌트로 범위를 좁혀 보세요.",
      note: `${filterNote} 스트릭은 끊기지만 탐색 힌트는 계속 쓸 수 있습니다.`,
      accent: "#ff7e8c",
    };
  }
...
function buildSubjectLine(word, eventType) {
  ...
  const axis = CATEGORY_AXES[word.category] || "형상 / 감정 / 표면";

  if (eventType === "hint") {
    return `"${word.label}" 쪽 냄새가 나요. ${getCategoryDisplay(word.category)} 결을 따라오면 ${motif}가 먼저 붙고, ${axis} 축이 움직일 거예요.`;
  }
  ...
  if (eventType === "category") {
    return `"${getCategoryDisplay(word.category)}" 쪽 공기가 짙어졌어요. ${word.label} 같은 말이 지금은 더 잘 먹힐 거예요.`;
  }
...
function buildDialogueDirective(word, eventType) {
  ...
  if (eventType === "hint") {
    ...
    return `${getCategoryDisplay(word.category)} 계열 · ${word.id.length}글자 · ${prefix} 시작을 기억하세요.`;
  }
  ...
  if (eventType === "category") {
    return `${getCategoryDisplay(word.category)} 포커스 활성화. ${brief?.directive || "같은 계열 단어를 계속 밀어 보세요."}`;
  }
...
function buildDialogueNote(word, eventType) {
  ...
  if (eventType === "hint") {
    return `힌트 고정 중입니다. 분위기 축은 ${word.mood}이고, 표면 단서는 ${motif}, 변형 축은 ${axis}입니다.`;
  }
  ...
  if (eventType === "category") {
    return `선택 카테고리: ${getCategoryDisplay(word.category)}. 변형 축은 ${axis}. 미해금 단어를 먼저 밀면 흐름이 더 잘 이어집니다.`;
  }

  return `${getCategoryDisplay(word.category)} 반응 고정. 표면 단서: ${motif}. 변형 축: ${axis}.`;
}
...
function buildNextPush(word) {
  ...
  if (!related.length) {
    return `${getCategoryDisplay(word.category)} 계열은 거의 닫혔습니다. 다른 카테고리로 옮겨도 됩니다.`;
  }
  ...
  return `${getCategoryDisplay(word.category)} 계열에서 ${pool.map((entry) => entry.label).join(", ")} 쪽으로 더 밀면 결이 자연스럽게 이어집니다.`;
}

function renderStageAliases(word) {
  const aliases = getAliasSet(word);
  if (!aliases.length) {
    els.stageAliases.innerHTML = `<span class="tag-chip">별칭 없음</span>`;
    return;
  }

  els.stageAliases.innerHTML = aliases
    .slice(0, 6)
    .map((alias) => `<span class="tag-chip">${escapeHtml(alias)}</span>`)
    .join("");
}

function getAliasSet(word) {
  const rawAliases = LOCALIZED_ALIASES[word.id] || [];
  return Array.from(new Set([word.id, ...rawAliases])).filter(Boolean);
}

function formatAliases(word) {
  return getAliasSet(word).join(" · ");
}

function getCategoryDisplay(category) {
  return CATEGORY_DISPLAY[category] || toLabel(category);
}

function buildDriftAxis(word) {
  return `${CATEGORY_AXES[word.category] || "형상 / 감정 / 표면"} · ${word.mood}`;
}

function buildRecentTrigger(word) {
  const lastInput = state.save.lastInput || word.id;
  const eventLabel = EVENT_LABELS[state.save.lastEventType] || EVENT_LABELS.boot;
  return `${eventLabel} · ${lastInput} · ${getPromptFragments(word.promptDelta, 2)}`;
}

function computeResonanceScore() {
  const attempts = state.save.attempts || 0;
  const successes = state.save.successfulAttempts || 0;
  const accuracy = attempts ? Math.round((successes / attempts) * 100) : 0;
  const streakBonus = Math.min((state.save.streak || 0) * 6, 24);
  const hintPenalty = Math.min((state.save.hintCount || 0) * 2, 18);
  return Math.max(0, Math.min(100, accuracy + streakBonus - hintPenalty));
}

function buildEventFeedCopy(entry, word) {
  const eventLabel = EVENT_LABELS[entry.type] || entry.type;
  if (!word) {
    return entry.input ? `${eventLabel} · ${entry.input}` : `${eventLabel} · 비단어 이벤트`;
  }

  return `${word.label} · ${getCategoryDisplay(word.category)} · ${entry.input || word.id}`;
}
  ...
  state.save.eventHistory = [
    {
      ...
      seq: (state.save.eventHistory?.[0]?.seq || 0) + 1,
    },
    ...(state.save.eventHistory || []),
  ].slice(0, 14);
}
...
function buildHintLine(word) {
  ...
  return `${getCategoryDisplay(word.category)} 계열입니다. ${word.id.length}글자이며 ${prefix}로 시작합니다.${extra}`;
}
...
function maybeCelebrateCategory(categorySlug) {
  ...
  pushLog("success", `카테고리 완성: ${getCategoryDisplay(categorySlug)}`, `${getCategoryDisplay(categorySlug)} 카테고리의 모든 상태를 해금했습니다.`);
}
...
function focusCategory(categorySlug) {
  ...
  state.save.categoryJumpCount += 1;
  ...
  if (discoveredInCategory.length) {
    ...
    pushLog("info", "카테고리 점프", `${getCategoryDisplay(categorySlug)} 카테고리의 해금 상태로 이동했습니다.`);
  } else {
    ...
    if (lockedInCategory.length) {
      ...
      pushLog("info", "카테고리 점프", `${getCategoryDisplay(categorySlug)} 카테고리는 아직 미해금입니다. 첫 힌트를 준비했습니다.`);
    }
  }
