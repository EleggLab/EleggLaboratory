...
export default function TarotHomeScreen({
  ...
  onNavigateReading: (type: TarotReadingType) => void;
  onNavigateResult: (type: TarotReadingType, cached: boolean) => void;
}): React.JSX.Element {
  ...
  const goReading = async (type: TarotReadingType): Promise<void> => {
    if (type === 'today') {
      ...
      if (saved) {
        onNavigateResult(type, true);
        return;
      }
    }
    onNavigateReading(type);
  };
