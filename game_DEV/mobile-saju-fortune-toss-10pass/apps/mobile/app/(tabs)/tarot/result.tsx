...
function summaryFor(type: TarotReadingType): string {
  if (type === 'today') return '오늘 하루의 기본 결을 짧고 선명하게 읽는 리딩이에요.';
  if (type === 'love') return '감정의 결, 관계의 거리감, 지금 필요한 태도를 함께 읽어봐요.';
  if (type === 'money') return '돈의 흐름과 지출 감각, 지금 잡아야 할 기준을 함께 살펴봐요.';
  if (type === 'relationship') return '사람 사이의 온도와 오해가 생기는 지점을 차분히 짚어봐요.';
  return '집중력과 성과의 방향, 지금 붙잡아야 할 공부 리듬을 정리해봐요.';
}
  ...
  return (
    <ScreenScroll background={BACKGROUNDS.tarot} contentContainerStyle={[commonStyles.screen, styles.container]}>
      <View style={[commonStyles.hero, styles.header]}>
        ...
        <Text style={styles.heroSub}>{summaryFor(type)}</Text>
      </View>
        ...
        <SectionCard title="뽑은 카드">
          <View style={styles.metaRow}>
            <View style={styles.metaChip}>
              <Text style={styles.metaChipText}>{titleFor(type)}</Text>
            </View>
            <View style={styles.metaChip}>
              <Text style={styles.metaChipText}>카드 {drawnDefs.length}장</Text>
            </View>
          </View>
          <View
            style={[
      ...
      {readingText ? (
        <SectionCard title="풀이">
          <Text style={styles.readingLead}>한 줄 결론보다 먼저, 반복되는 키워드와 카드의 자리부터 읽어보세요.</Text>
