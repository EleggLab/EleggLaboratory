...
function collectIssues(strict) {
  const issues = [];
  const appName = readEnv('TOSS_APP_NAME', 'magic-toss');
  const consoleAppName = readEnv('TOSS_CONSOLE_APP_NAME', appName);
  const displayName = readEnv('TOSS_BRAND_DISPLAY_NAME', '오늘의 마법수업');
  const primaryColor = readEnv('TOSS_BRAND_PRIMARY_COLOR', '#1C58F2');
  const iconUrl = readEnv('TOSS_BRAND_ICON_URL', 'https://placehold.co/600x600/png?text=Magic+Toss');
  ...
  // Product guardrails for this project:
  // - completely free
  // - no backend/server dependency
  const forbiddenIfSet = [
    'TOSS_AD_SLOT_ID',
    'TOSS_AD_UNIT_ID',
    'ADMOB_APP_ID',
    'ADMOB_UNIT_ID',
  ...
  for (const key of forbiddenIfSet) {
    const value = readEnv(key, '');
    if (value) {
      issues.push(`\`${key}\` must be empty for free/no-server mode.`);
    }
  }
...
if (issues.length === 0) {
  console.log(strict ? 'Magic Toss release env validation passed.' : 'Magic Toss env sanity check passed.');
  process.exit(0);
}

console.error(strict ? 'Magic Toss release env validation failed:' : 'Magic Toss env sanity check found issues:');
